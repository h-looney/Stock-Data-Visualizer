...
symbol = "IBM"
url = f'https://www.alphavantage.co/query?function=TIME_SERIES_WEEKLY&symbol=IBM&interval=5min&apikey=AU4UXQO6WFYENR4J'
r = requests.get(url)
data = r.json()
i = 0
Highinput = []
...
while date_2s != date_1s:
    ...
    try:
        opendata = data['Weekly Time Series'][dt]['1. open']
        highdata = data['Weekly Time Series'][dt]['2. high']
        lowdata = data['Weekly Time Series'][dt]['3. low']
        closedata = data['Weekly Time Series'][dt]['4. close']
